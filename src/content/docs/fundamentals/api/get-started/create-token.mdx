...
```json
{
	...
	"messages": [
		{
			"code": 10000,
			"message": "This API Token is valid and inactive",
			"type": null
		}
	]
}
...
New API tokens use the `cfut_` prefixed [scannable format](/fundamentals/api/get-started/token-formats/), which allows credential scanning tools to detect leaked tokens.
